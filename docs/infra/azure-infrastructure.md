...
- Canonical format (new resources): `{service}-hoops-{env}`
- Function Apps: `func-hoops-{env}` (example: `func-hoops-prod`)
- Resource groups: `rg-hoops-{env}`
- Key Vaults: `kv-hoops-{env}`
- SQL Servers: `sql-hoops-{env}`
- SQL Databases: descriptive name scoped to environment (example: `csbchoops-prod`)
- Storage Accounts: `sthoops{env}` or `sthoops{env}{nn}` (example: `sthoopsprod`)
  - Azure storage account naming rules require lowercase alphanumeric characters only (no hyphens).

### Legacy Naming Reconciliation Policy

- Existing resources are not renamed solely for naming consistency.
- Legacy names remain valid until a natural replacement event (SKU migration, regional move, major re-platform, or security-driven rotation).
- All net-new resources must follow the canonical format above.
- Where legacy names exist, document both names during transition:
  - Physical Azure name: current deployed resource name
  - Logical/canonical name: the standardized `{service}-hoops-{env}` identifier used in documentation and planning
