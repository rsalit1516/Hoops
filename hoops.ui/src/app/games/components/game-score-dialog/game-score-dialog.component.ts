...
export class GameScoreDialogComponent implements OnDestroy {
  ...
  gameScoreForm = this.fb.nonNullable.group({
    homeTeamScore: [
      ...
      [
        ...
        Validators.min(0),
        Validators.max(150),
        GameScoreDialogComponent.integerValidator,
      ],
    ],
    visitorTeamScore: [
      0,
  ...
  constructor(
    ...
    console.log('GameScoreDialogComponent initial game:', initial);
    if (initial) {
  ...
  onSubmitClick() {
    ...
    this.gameService
      ...
      .subscribe({
        next: () => {
          this.snackBar.open('Scores saved', undefined, { duration: 2000 });
          this.dialogRef.close();
        },
        error: (err) => {
          console.error(err);
          this.saving = false;
          this.snackBar.open('Failed to save scores', 'Dismiss', {
            duration: 3000,
          });
        },
      });
